...
const DROPDOWN_BASE = "absolute top-full mt-2 z-50 bg-white dark:bg-neutral-900 border border-gray-100 dark:border-white/10 rounded-xl shadow-xl overflow-hidden min-w-max";
        ...
        <div className={`${DROPDOWN_BASE} ${DROPDOWN_ALIGN[align]} ${status === "closing" ? "anim-scale-out" : "anim-scale-in"}`}>
          ...
          {options.map((opt) => {
            ...
            return (
              ...
              >
                {dot && <span className={`w-2 h-2 rounded-full shrink-0 ${opt.dot || "bg-gray-400 dark:bg-neutral-600"}`} />}
  ...
  return (
    <div ref={ref} className="relative ml-1 inline-block">
      <button
        ...
        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white dark:bg-neutral-900 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 text-xs hover:border-gray-300 dark:hover:border-white/20 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-neutral-800 active:opacity-70 transition-all duration-150 cursor-pointer focus:outline-none"
      >
...
export default function RadiusSelector({
  ...
  const formatOptions = [
    { value: "", label: "All formats", dot: "bg-gray-400 dark:bg-neutral-600" },
  ...
  return (
    <>
      {toast && (
        <div
          className="fixed z-50 px-3 py-2 bg-white dark:bg-neutral-900 border border-gray-100 dark:border-white/8 rounded-lg text-sm text-gray-900 dark:text-white font-medium shadow-lg whitespace-nowrap pointer-events-none"
